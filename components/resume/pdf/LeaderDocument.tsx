...
/**
 * The `leader` resume — for managers and senior+ ICs who lead. Scope-and-
 * outcome: a scope strip on every role surfaces team size, budget, and
 * reporting line; bullets are fewer and outcome-weighted; role titles take the
 * serif so each reads as a heading. More air than `ic-technical`.
 */
...
const s = StyleSheet.create({
  ...
  summary: { fontSize: 9.75, color: BODY, lineHeight: 1.58 },
  ...
  roleCompany: { fontSize: 9.5, color: COMPANY, marginTop: 3, lineHeight: 1.35 },
  scope: {
    fontSize: 8.75,
    fontFamily: SANS,
    fontWeight: 600,
...
export function LeaderDocument({ data }: { data: IResumeDocument }) {
  const contactLine = [
    data.contact.email,
    data.contact.phone,
    ...
    <Document title={`${data.name} Resume`} author={data.name}>
      <Page size="LETTER" style={s.page}>
        ...
        {data.speaking && data.speaking.length > 0 ? (
          <View style={s.section} wrap={false}>
            <SectionTitle>Speaking &amp; Advisory</SectionTitle>
