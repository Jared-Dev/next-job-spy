...
/**
 * The `ic-technical` resume — the single source of truth for both the
 * on-screen preview and the exported/stored PDF, so the two can never drift. A
 * taut layout: positioning summary, a competency band high up, then
 * reverse-chronological experience tapering on older roles.
 *
 * Content flows naturally across pages (each role is atomic, never split), so
 * the document is as long as it needs to be rather than a fixed page count.
 */
...
const s = StyleSheet.create({
  ...
  summary: { fontSize: 9.75, color: BODY, lineHeight: 1.55 },
...
export function IcTechnicalDocument({ data }: { data: IResumeDocument }) {
  const contactLine = [
    data.contact.email,
    data.contact.phone,
        ...
        {data.projects && data.projects.length > 0 ? (
          <View style={s.section} wrap={false}>
            ...
            {data.projects.map((project) => (
              <Text key={project.title} style={s.entry}>
