...
export async function getProfileAction(): Promise<IProfile | null> {
  ...
  return {
    ...
    forFun: row.forFun ?? undefined,
    dismissedSkills: row.dismissedSkills ?? undefined,
    cvStories: row.cvStories ?? undefined,
    cvInterviewTranscript: row.cvInterviewTranscript ?? undefined,
...
export async function saveProfileAction(input: unknown): Promise<void> {
  ...
  const values = {
    ...
    forFun: data.forFun ?? null,
    dismissedSkills: data.dismissedSkills ?? null,
    cvStories: data.cvStories ?? null,
    cvInterviewTranscript: data.cvInterviewTranscript ?? null,
