...
- Profile date format: startDate/endDate in the profile use ISO "YYYY-MM" and denote the NAMED month, not the month before it. Map the two-digit month directly to its name and never adjust by one: 01=Jan, 02=Feb, 03=Mar, 04=Apr, 05=May, 06=Jun, 07=Jul, 08=Aug, 09=Sep, 10=Oct, 11=Nov, 12=Dec. "2024-10" is "Oct 2024". "2026-05" is "May 2026". "2018-06" is "Jun 2018". Do not interpret "YYYY-MM" as "the moment just before YYYY-MM". Do not infer the month from the candidate's tenure length or surrounding context when the profile already supplies the month, use what the profile says.
- If the candidate's profile only supplies a year for one end of the range, infer the most plausible month from context (typical start of role, season hinted by surrounding facts) or fall back to "Jan" / "Dec"; never emit a bare year. Never use an en dash or em dash anywhere in the document.
