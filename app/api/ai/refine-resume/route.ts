...
export async function POST(request: Request) {
  ...
  // Stable across refines for this user, so it rides the Agent SDK's
  // auto-cached system prompt. The base resume to revise and the refinement
  // instruction live in the user prompt below.
...
Now produce the revised resume as JSON per the system rules.`;
  ...
  try {
    ...
    const resume = enforceRoleCap(parseJsonFromAgent(text, ResumeDocumentSchema));
